...
class ThermometerTemperatureMonitor: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    override init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(disConnectToDevice), name: notificationName.disconnected.notification, object: nil)
    }
    ...
    @objc func disConnectToDevice(){
        centralManager.cancelPeripheralConnection(DataSingleton.displayPeripheral!.peripheral)
    }
    ...
    func getHeartBPMData(characteristic: CBCharacteristic, error: Error?,peripheral: CBPeripheral) {
        ...
        if ((buffer[0] & 0x01) == 0) {
            ...
            if mantissa >= 3950 && isShowWarning {
