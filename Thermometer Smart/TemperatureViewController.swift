...
class TemperatureViewController: UIViewController, TemperatureView {
    ...
    func setData(){
        guard DataSingleton.displayPeripheral != nil else { return }
        lbNameDevice.text = DataSingleton.displayPeripheral?.name
        lbMac.text = DataSingleton.displayPeripheral?.macAddress
    }
